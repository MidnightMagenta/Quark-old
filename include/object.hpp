...
namespace qrk
{
			...
			loadFinished(false), asyncLoad(async)
		{
			if(async)
			{
				...
				std::thread loadThread(
					std::bind(&qrk::Object::LoadObjectAsync, this, std::placeholders::_1, std::placeholders::_2), 
					path, 
					std::move(promisedData));
				loadThread.detach();
			}
			else
			{
	...
	public:
			...
			color({ 1.f, 1.f, 1.f, 1.f })
		{
			...
			glBufferData(GL_ARRAY_BUFFER, objectData->data.size() * sizeof(GLfloat), objectData->data.data(), GL_STATIC_DRAW);
		}
