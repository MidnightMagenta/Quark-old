...
void qrk::Debug::OpenLogFile(std::string path)
{
	...
	if (!logFile.is_open()) { MessageBox(0, "Failed to open log file", "Warning", MB_OK | MB_ICONEXCLAMATION); }
}
void qrk::Debug::Log(std::string log)
{
	...
	time_t tt;
	time(&tt);
	logFile << "[INFO] Time: " << std::put_time(localtime(&tt), "%c") << " Message: " << log << std::endl;
}
void qrk::Debug::LogWarning(std::string warning)
{
	...
	time_t tt;
	time(&tt);
	logFile << "[WARNING] Time: " << std::put_time(localtime(&tt), "%c") << " Message: " << warning << std::endl;
}
void qrk::Debug::LogError(std::string error)
{
	...
	time_t tt;
	time(&tt);
	logFile << "[ERROR] Time: " << std::put_time(localtime(&tt), "%c") << " Message: " << error << std::endl;
}